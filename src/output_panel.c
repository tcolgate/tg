...
static cairo_pattern_t *black, *white, *red, *green, *blue, *blueish, *yellow,
    *goldenrod;
...
static void draw_graph(double a, double b, cairo_t *c,
                       struct processing_buffers *p, GtkWidget *da) {
  GtkAllocation temp;
  ...
  for (n = 0; n < 2 * width; n++) {
    ...
    if (p->waveform[j] <= 0)
      ...
      y = p->waveform[j] * 0.4 / p->waveform_max;
...
#ifdef DEBUG
static void draw_debug_graph(double a, double b, cairo_t *c,
                             struct processing_buffers *p, GtkWidget *da) {
  if (!p->debug)
    return;
  ...
  int first = 1;
  for (i = 0; i < width; i++) {
    if (round(a + i * (b - a) / width) !=
        round(a + (i + 1) * (b - a) / width)) {
...
static double draw_watch_icon(cairo_t *c, int signal, bool partial, bool happy,
                              bool light) {
  cairo_set_line_width(c, 3);
  cairo_set_source(c, happy ? green : red);
  cairo_move_to(c, OUTPUT_WINDOW_HEIGHT * 0.5, OUTPUT_WINDOW_HEIGHT * 0.5);
  cairo_line_to(c, OUTPUT_WINDOW_HEIGHT * 0.75,
                OUTPUT_WINDOW_HEIGHT * (0.75 - 0.5 * happy));
  cairo_move_to(c, OUTPUT_WINDOW_HEIGHT * 0.5, OUTPUT_WINDOW_HEIGHT * 0.5);
  cairo_line_to(c, OUTPUT_WINDOW_HEIGHT * 0.35,
                OUTPUT_WINDOW_HEIGHT * (0.65 - 0.3 * happy));
  cairo_stroke(c);
  cairo_arc(c, OUTPUT_WINDOW_HEIGHT * 0.5, OUTPUT_WINDOW_HEIGHT * 0.5,
            OUTPUT_WINDOW_HEIGHT * 0.4, 0, 2 * M_PI);
  cairo_stroke(c);
  int l = OUTPUT_WINDOW_HEIGHT * 0.8 / (2 * NSTEPS - 1);
  int i;
  cairo_set_line_width(c, 1);
  for (i = 0; i < signal; i++) {
    cairo_move_to(c, OUTPUT_WINDOW_HEIGHT + 0.5 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.9 - 2 * i * l);
    cairo_line_to(c, OUTPUT_WINDOW_HEIGHT + 1.5 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.9 - 2 * i * l);
    cairo_line_to(c, OUTPUT_WINDOW_HEIGHT + 1.5 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.9 - (2 * i + 1) * l);
    cairo_line_to(c, OUTPUT_WINDOW_HEIGHT + 0.5 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.9 - (2 * i + 1) * l);
    cairo_line_to(c, OUTPUT_WINDOW_HEIGHT + 0.5 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.9 - 2 * i * l);
    cairo_stroke_preserve(c);
    if (i < signal - 1 || !partial)
      cairo_fill(c);
  }
  if (light) {
    int l = OUTPUT_WINDOW_HEIGHT * 0.15;
    cairo_set_line_width(c, 2);
    cairo_move_to(c, OUTPUT_WINDOW_HEIGHT * 0.5 - 0.5 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.2);
    cairo_line_to(c, OUTPUT_WINDOW_HEIGHT * 0.5 - 0.5 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.2 + l);
    cairo_line_to(c, OUTPUT_WINDOW_HEIGHT * 0.5,
                  OUTPUT_WINDOW_HEIGHT * 0.2 + l);
    cairo_move_to(c, OUTPUT_WINDOW_HEIGHT * 0.5 - 0.2 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.2 + 1);
    cairo_line_to(c, OUTPUT_WINDOW_HEIGHT * 0.5 + 0.8 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.2 + 1);
    cairo_move_to(c, OUTPUT_WINDOW_HEIGHT * 0.5 + 0.3 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.2 + 1);
    cairo_line_to(c, OUTPUT_WINDOW_HEIGHT * 0.5 + 0.3 * l,
                  OUTPUT_WINDOW_HEIGHT * 0.2 + l + 1);
    cairo_stroke(c);
  }
...
static gboolean output_draw_event(GtkWidget *widget, cairo_t *c,
  ...
  double x = draw_watch_icon(c, snst->signal, snst->amp <= 0,
                             snst->signal >= (snst->calibrate ? NSTEPS : 1),
                             snst->is_light);
  ...
  cairo_text_extents(c, "0", &extents);
  double y =
      (double)OUTPUT_WINDOW_HEIGHT / 2 - extents.y_bearing - extents.height / 2;
...
static void expose_waveform(struct output_panel *op, GtkWidget *da, cairo_t *c,
  ...
  for (i = 10; i < 360; i += 10) {
    ...
    if (t > .001 * NEGATIVE_SPAN)
      continue;
    int x = round(width * (NEGATIVE_SPAN - 1000 * t) /
                  (NEGATIVE_SPAN + POSITIVE_SPAN));
  ...
  cairo_move_to(c, width - extents.x_advance - font / 4,
                margin + fextents.ascent);
  ...
  if (p) {
    ...
    if (pulse > 0) {
      int x = round((NEGATIVE_SPAN - pulse / span) * width /
...
static int get_tic(struct processing_buffers *p) { return p->tic; }

static int get_toc(struct processing_buffers *p) { return p->toc; }
...
static gboolean tic_draw_event(GtkWidget *widget, cairo_t *c,
                               struct output_panel *op) {
  UNUSED(widget);
...
static gboolean toc_draw_event(GtkWidget *widget, cairo_t *c,
                               struct output_panel *op) {
  UNUSED(widget);
  expose_waveform(op, op->toc_drawing_area, c, goldenrod, get_toc,
                  get_toc_pulse);
  return FALSE;
}

static gboolean period_draw_event(GtkWidget *widget, cairo_t *c,
                                  struct output_panel *op) {
  UNUSED(widget);
  cairo_init(c);
  ...
  if (p) {
    ...
    cairo_move_to(c,
                  (p->tic - a - NEGATIVE_SPAN * .001 * snst->sample_rate) *
                      width / p->period,
                  0);
    cairo_line_to(c,
                  (p->tic - a - NEGATIVE_SPAN * .001 * snst->sample_rate) *
                  ...
                  height);
    cairo_line_to(c,
                  (p->tic - a + POSITIVE_SPAN * .001 * snst->sample_rate) *
                      width / p->period,
                  height);
    cairo_line_to(c,
                  (p->tic - a + POSITIVE_SPAN * .001 * snst->sample_rate) *
    ...
    cairo_move_to(c,
                  (toc - a - NEGATIVE_SPAN * .001 * snst->sample_rate) * width /
                      p->period,
                  0);
    cairo_line_to(c,
                  (toc - a - NEGATIVE_SPAN * .001 * snst->sample_rate) * width /
                  ...
                  height);
    cairo_line_to(c,
                  (toc - a + POSITIVE_SPAN * .001 * snst->sample_rate) * width /
                      p->period,
                  height);
    cairo_line_to(c,
                  (toc - a + POSITIVE_SPAN * .001 * snst->sample_rate) * width /
...
static double get_beatscale(GtkScaleButton *b) {
  ...
  const double ω =
      pow(scale_max / scale_min, -1.0 / (zoom_max - zoom_min)) / scale_max;
...
static gboolean paperstrip_draw_event(GtkWidget *widget, cairo_t *c,
                                      struct output_panel *op) {
  int i;
  char s[32];
  ...
  uint64_t time =
      snst->timestamp ? snst->timestamp : get_timestamp(snst->is_light);
  cairo_text_extents_t extents;
  ...
  // Width in pixels of main chart area, which corresponds to chart_width in
  // samples
  ...
  GtkAdjustment *scroll =
      op->vertical_layout ? gtk_scrollable_get_vadjustment(
                                GTK_SCROLLABLE(op->paperstrip_drawing_area))
  ...
  if (snst->pb) { // pb == NULL means no rate, beat error, etc.
    ...
    if (slope > -2 && slope < 2) {
      ...
      cairo_set_source(c, blue);
      /* X intercept of line starting at lower left corner, in quarter widths
       * left+4 is intercept from lower right corner.  Intercepts at top
       * left/right corners are always 0 and 4.  We need to draw lines from the
       * lesser of the left corner intercepts to the greater of the right corner
       * intercepts to cover the width of the chart at the top and botom.  */
  ...
  if (snst->amps_count) {
    ...
    for (int i = snst->amps_count; i > 0; i--) {
      ...
      if (snst->amps_time[idx] > time)
        continue;
      const double t = (time - snst->amps_time[idx]) / beat_length;
      const double a =
          (snst->la * snst->amps[idx] - 135.0) / (360 - 135) * width;
      if (t > height)
        break;
  ...
  if (snst->events_count) {
    ...
    uint64_t prev_event =
        snst->events[snst->events_wp];         // Start with first event
    for (i = snst->events_count; i > 0; i--) { // Scan order is newest to oldest
      ...
      if (residual > beat_length / 2)
        residual -= beat_length;
  ...
  for (i = snst->events_count; i > 0; i--) {
    ...
    debug("point %2d: %7lu, cycle %.1f, offset %.1f ms, chart phase = %.1f ms, "
          "column %.0f\n",
          idx, event, cycles, s2ms(snst, offsets[idx] + display_offset),
          s2ms(snst, chart_phase), column);
#endif
  }
  cairo_stroke(c);
  ...
  cairo_text_extents(c, s, &extents);
  cairo_font_extents(c, &fextents);
  cairo_move_to(c, (width - extents.width) / 2 - extents.x_bearing,
                (height - 25.5) + fextents.ascent - fextents.height);
  cairo_show_text(c, s);
...
#ifdef DEBUG
static gboolean debug_draw_event(GtkWidget *widget, cairo_t *c,
                                 struct output_panel *op) {
  UNUSED(widget);
  cairo_init(c);
...
static void handle_zoom(GtkScaleButton *b, gdouble value,
                        struct output_panel *op) {
  UNUSED(value);
  struct display *ssd = op->snst->d;
  if (!ssd)
    return; // Maybe chart hasn't been displayed even once yet?
  ...
  gtk_widget_set_visible(op->zoom_orig_button,
                         gtk_scale_button_get_value(b) != zoom_mid);
  gtk_widget_queue_draw(op->paperstrip_drawing_area);
}
...
static void set_orientation(GtkWidget *widget, bool vertical) {
  gtk_orientable_set_orientation(GTK_ORIENTABLE(widget),
                                 vert_to_orient(vertical));
}

static void paperstrip_size_allocate(GtkWidget *self, GtkAllocation *a,
                                     struct output_panel *op) {
  guint width, height;
  if (op->vertical_layout) {
    width = a->width;
...
static GtkWidget *create_paperstrip(struct output_panel *op, bool vertical) {
  ...
  gtk_layout_set_size(GTK_LAYOUT(op->paperstrip_drawing_area),
                      vertical ? 150 : op->snst->event_age,
                      vertical ? op->snst->event_age : 150);
  op->paperstrip_scolled_window = gtk_scrolled_window_new(NULL, NULL);
  gtk_container_add(GTK_CONTAINER(op->paperstrip_scolled_window),
                    op->paperstrip_drawing_area);
  gtk_scrolled_window_set_min_content_height(
      GTK_SCROLLED_WINDOW(op->paperstrip_scolled_window), 150);
  gtk_scrolled_window_set_min_content_width(
      GTK_SCROLLED_WINDOW(op->paperstrip_scolled_window), 150);
  gtk_scrolled_window_set_overlay_scrolling(
      GTK_SCROLLED_WINDOW(op->paperstrip_scolled_window), FALSE);
  gtk_range_set_inverted(
      GTK_RANGE(gtk_scrolled_window_get_hscrollbar(
          GTK_SCROLLED_WINDOW(op->paperstrip_scolled_window))),
      TRUE);
  gtk_container_add(GTK_CONTAINER(overlay), op->paperstrip_scolled_window);
  g_signal_connect(op->paperstrip_drawing_area, "draw",
                   G_CALLBACK(paperstrip_draw_event), op);
  gtk_widget_set_events(op->paperstrip_drawing_area, GDK_EXPOSURE_MASK);
  g_signal_connect(op->paperstrip_drawing_area, "size-allocate",
                   G_CALLBACK(paperstrip_size_allocate), op);
  ...
  op->zoom_button =
      gtk_scale_button_new(GTK_ICON_SIZE_BUTTON, zoom_min, zoom_max, 1,
                           (const char *[]){"zoom-in-symbolic", NULL});
  gtk_scale_button_set_value(GTK_SCALE_BUTTON(op->zoom_button), zoom_mid);
  set_orientation(op->zoom_button, vertical);
  g_signal_connect(op->zoom_button, "value-changed", G_CALLBACK(handle_zoom),
                   op);
  gtk_box_pack_start(GTK_BOX(box), op->zoom_button, FALSE, FALSE, 0);

  op->zoom_orig_button = gtk_button_new_from_icon_name("zoom-original-symbolic",
                                                       GTK_ICON_SIZE_BUTTON);
  gtk_button_set_relief(GTK_BUTTON(op->zoom_orig_button), GTK_RELIEF_NONE);
  g_signal_connect(op->zoom_orig_button, "clicked",
                   G_CALLBACK(handle_zoom_original), op);
  ...
  op->left_button = gtk_button_new_from_icon_name(
      vertical ? "pan-start-symbolic" : "pan-down-symbolic",
  ...
  if (op->computer) {
    ...
    g_signal_connect(op->clear_button, "clicked",
                     G_CALLBACK(handle_clear_trace), op);
  ...
  g_signal_connect(center_button, "clicked", G_CALLBACK(handle_center_trace),
                   op);

  // > button
  op->right_button = gtk_button_new_from_icon_name(vertical ? "pan-end-symbolic"
                                                            : "pan-up-symbolic",
                                                   GTK_ICON_SIZE_LARGE_TOOLBAR);
...
static GtkWidget *create_waveforms(struct output_panel *op, bool vertical) {
  ...
  g_signal_connect(op->tic_drawing_area, "draw", G_CALLBACK(tic_draw_event),
                   op);
  ...
  g_signal_connect(op->toc_drawing_area, "draw", G_CALLBACK(toc_draw_event),
                   op);
  ...
  g_signal_connect(op->period_drawing_area, "draw",
                   G_CALLBACK(period_draw_event), op);
  ...
  g_signal_connect(op->debug_drawing_area, "draw", G_CALLBACK(debug_draw_event),
                   op);
 ...
 */
static void place_displays(struct output_panel *op, GtkWidget *paperstrip,
                           GtkWidget *waveforms, bool vertical) {
  ...
  gtk_paned_pack1(GTK_PANED(op->displays), paperstrip, vertical ? FALSE : TRUE,
                  FALSE);
  ...
  gtk_image_set_from_icon_name(GTK_IMAGE(left_arrow),
                               vertical ? "pan-start-symbolic"
                                        : "pan-down-symbolic",
                               GTK_ICON_SIZE_LARGE_TOOLBAR);
  GtkWidget *right_arrow = gtk_button_get_image(GTK_BUTTON(op->right_button));
  gtk_image_set_from_icon_name(
      GTK_IMAGE(right_arrow), vertical ? "pan-end-symbolic" : "pan-up-symbolic",
      GTK_ICON_SIZE_LARGE_TOOLBAR);
  ...
  gtk_scrolled_window_set_propagate_natural_width(
      GTK_SCROLLED_WINDOW(op->paperstrip_scolled_window), vertical);
...
void set_panel_layout(struct output_panel *op, bool vertical) {
  ...
  gtk_widget_destroy(op->displays);
  op->displays = NULL;
  place_displays(op, op->paperstrip_box, op->waveforms_box, vertical);
...
struct output_panel *init_output_panel(struct computer *comp,
                                       struct snapshot *snst, int border,
                                       bool vertical) {
  ...
  gtk_box_pack_start(GTK_BOX(op->panel), op->output_drawing_area, FALSE, TRUE,
                     0);
  g_signal_connect(op->output_drawing_area, "draw",
                   G_CALLBACK(output_draw_event), op);
