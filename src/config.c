...
static gboolean g_key_file_save_to_file(GKeyFile *key_file,
...
static void save_filter_chain(struct main_window *w) {
  ...
  g_key_file_set_integer_list(w->config_file, "filter_chain", "frequency",
                              freqs, n);
  ...
  g_key_file_set_boolean_list(w->config_file, "filter_chain", "enabled",
                              enabled, n);
}

static void load_filter_chain(struct main_window *w) {
  struct filter_chain *chain =
      filter_chain_init(w->audio_rate ? (double)w->audio_rate : PA_SAMPLE_RATE);
  ...
  if (g_key_file_has_group(w->config_file, "filter_chain")) {
    ...
    types = g_key_file_get_integer_list(w->config_file, "filter_chain", "type",
                                        &n, &e);
    if (e)
      goto fail;
    freqs = g_key_file_get_integer_list(w->config_file, "filter_chain",
                                        "frequency", &nn, &e);
    if (e || n != nn)
      goto fail;
    qs = g_key_file_get_double_list(w->config_file, "filter_chain", "q", &nn,
                                    &e);
    if (e || n != nn)
      goto fail;
    gains = g_key_file_get_double_list(w->config_file, "filter_chain", "gain",
                                       &nn, &e);
    if (e || n != nn)
      goto fail;
    enabled = g_key_file_get_boolean_list(w->config_file, "filter_chain",
                                          "enabled", &nn, &e);
    if (e || n != nn)
      goto fail;
  ...
  filter_chain_set(chain, 0, HIGHPASS,
                   w->hpf_freq ? w->hpf_freq : FILTER_CUTOFF, M_SQRT1_2, 0);
...
void load_config(struct main_window *w) {
  ...
  w->config_file_name =
      g_build_filename(g_get_user_config_dir(), CONFIG_FILE_NAME, NULL);
  w->conf_data = malloc(sizeof(struct conf_data));
#define SETUP(NAME, PLACE, TYPE) w->conf_data->PLACE = w->PLACE;
  ...
  gboolean ret = g_key_file_load_from_file(w->config_file, w->config_file_name,
...
#define LOAD(NAME, PLACE, TYPE)                                                \
...

  {
    ...
    gchar **val = g_key_file_get_string_list(w->config_file, "tg", "positions",
                                             &positions_size, &e);
    if (e) {
      debug("Config: error loading field " #NAME "\n");
      g_error_free(e);
...
#define SAVE(NAME, PLACE, TYPE)                                                \
  ...
  g_key_file_set_string_list(w->config_file, "tg", "positions",
                             (const gchar *const *)w->positions,
                             w->positions_size);
