 ...
 * There are calls that need this from the audio callback thread, the GUI
 * thread, and the computer thread.  Having each thread pass it in correctly
 * would be really hard. It's better to maintain it in one place here in the
 * audio code.  Lacking class scope in C, we'll have to settle for static global
 * scope.  We only support once device at a time, so not supporting multiuple
 * audio contexts isn't much of a drawback.
 * */
static struct audio_context {
  ...
  //! Data callback will read, need to take care when modifying so as not to
  //! race.
 ...
 */
static void apply_biquad(struct biquad_filter *f, float *data,
                         unsigned int count) {
  unsigned int i;
...
/* Functions for working on a chain of biquad filters.  A chain contains zero or
 * more filters. It will grow as needed.  There are functions to insert, remove,
 * access, enable, and apply filters.  A filter may be in the chain but not be
 * enabled, which means it will be skipped when the chain is applied to audio.
 *
 * The chain is designed to be used simultaneously from two threads and
 * internally contains the necessary locking.  The manner in which the two
 * threads access the chain is different:  One thread is an audio processing
 * thread the other a control thread.
 *
 * The audio processing thread:
 *   Applies the filters to audio, but not does not change the chain itself.
 *   Has read/write access to the delay taps, but read-only access to all other
 * fields. Does not care about any values of unenabled filters. Will acquire the
 * chain's lock for its entire access to the filter. May hold the lock for a
 * longer time.
 *
 * The control thread:
 *   Has read/write access to the eveything in the chain, but should not access
 * the delay taps. Does not acquire the chain's lock for read-only accesses to
 * the chain. Can perform read/write access on an unenabled filter without a
 * lock (but can not delete the filter). Only acquires the lock for very short
 * periods of time.
 *
 * This design is meant to allow a read-time audio processing thread the least
 * chance of being blocked by access from the control thread.  The control
 * thread needs to lock to query the chain and when modifying it needs a brief
 * lock to insert new blank filter or disable an existing filter, which can then
 * be modified without a lock while disabled, and the a brief
...
/* Insert empty non-enabled filter at index.  The index should be at most 1
 * greater than the index of the last filter, i.e. no gaps.  (unsigned)-1 will
 * insert at the end.  Returns a
 * pointer to the new filters (or NULL on failure, which should not happena.) */
struct biquad_filter *filter_chain_insert(struct filter_chain *chain,
                                          unsigned index) {
  if (index == (unsigned)-1) {
    index = chain->count;
  ...
  if (chain->count == chain->max) {
    ...
    /* Doing this re-alloc while holding the lock is a flaw.  We should allocate
     * the memory with the lock released, then copy with it held.  We could even
     * copy everything but the delay taps with it not held.  */
    chain->filters =
        realloc(chain->filters, chain->max * sizeof(*chain->filters));
  }
  memcpy(&chain->filters[index + 1], &chain->filters[index],
         sizeof(*chain->filters) * (chain->count - index));
...
void filter_chain_remove(struct filter_chain *chain, unsigned index) {
  if (index >= chain->count)
    return;
  pthread_mutex_lock(&chain->mutex);
  memcpy(&chain->filters[index], &chain->filters[index + 1],
         sizeof(*chain->filters) * (chain->count - index - 1));
  chain->count--;
  pthread_mutex_unlock(&chain->mutex);
}

/* Return pointer to filter.  Returns NULL if i is past the end of available
 * filters */
const struct biquad_filter *filter_chain_get(const struct filter_chain *chain,
                                             unsigned index) {
  if (index >= chain->count)
    return NULL;
  return &chain->filters[index];
...
bool filter_chain_enable(struct filter_chain *chain, unsigned index,
                         bool enable) {
...
/* Program a filter */
static void _filter_set_coefficients(struct filter *f, enum bitype type,
                                     double f0_fS, double q, double gain) {
  switch (type) {
  case HIGHPASS:
    make_hpq(f, f0_fS, q);
    break;
  ...
  case BANDPASS:
    ...
    break;
  case NOTCH:
    make_notch(f, f0_fS, q);
    break;
  case ALLPASS:
    make_ap(f, f0_fS, q);
    break;
  case PEAK:
    make_peak(f, f0_fS, q, gain);
    break;
  default:
    break;
  }
...
/* Set or update a filter.  Takes the chain lock for enabled filters, but
 * locking isn't needed for disabled filters.  Returns false if the filter
 * didn't need to be updated.  */
bool filter_chain_set_filter(struct filter_chain *chain,
                             struct biquad_filter *filter, enum bitype type,
                             unsigned freq, double q, double gain) {
  if (filter->type == type && filter->frequency == freq && filter->bw == q &&
      filter->gain == gain)
    return false;
...
/* Adjusts existing filters to new sample rate.  Does nothing if rate is
 * unchanged. */
static bool filter_chain_rate(struct filter_chain *chain, double sample_rate) {
  ...
  for (i = 0; i < chain->count; i++) {
    ...
    _filter_set_coefficients(&filter->f, filter->type,
                             filter->frequency / sample_rate, filter->bw,
                             filter->gain);
  }
  pthread_mutex_unlock(&chain->mutex);
...
/* Apply filter chain to audio data.  Data is fixed as the global array
 * pa_buffers.  Process data over range [start, stop).  This might wrap.  */
static void filter_chain_apply(struct filter_chain *chain, float *data,
                               unsigned start, unsigned stop, unsigned size) {
  unsigned i;

  /* Apply each filter in turn on a single contiguous chunk of the ciruclar
     buffer, of which there is either 1 or 2 chunks. */
...
static int paudio_callback(const void *input_buffer, void *output_buffer,
  ...
  if (info->light) {
    ...
    if (info->channels == 1) {
      ...
      if (len < frame_count)
        memcpy(pa_buffers, input_samples + len,
               (frame_count - len) * sizeof(*pa_buffers));
    } else {
      for (i = 0; i < len; i++)
      ...
      if (len < frame_count)
        for (i = len; i < frame_count; i++)
          pa_buffers[i - len] =
              input_samples[2u * i] + input_samples[2u * i + 1u];
    }
  ...
  filter_chain_apply(info->chain, pa_buffers, write_pointer, wp,
                     pa_buffer_size);
...
static PaError open_stream(PaDeviceIndex index, unsigned int rate, bool light,
                           PaStream **stream) {
  PaError err;
  ...
  err = Pa_OpenStream(
      ...
      NULL, rate, paFramesPerBufferUnspecified, paNoFlag, paudio_callback,
      &actx.info);
  return err;
}
 ...
 * @param[in,out] normal_sr Desired rate, or zero for default.  Rate used on
 * return.
 * @param[out] real_sr Actual exact rate received, might be different than
 * nominal_sr.
 * @param chain The filter chain for the audio.  NULL continues to use the
 * existing chain or creates an empty one. When changing devices after the first
 * open, only NULL is supported.
 ...
 */
int set_audio_device(int device, int *nominal_sr, double *real_sr,
                     struct filter_chain *chain, bool light) {
  PaError err;
...
error:
  ...
  error("Error opening audio device '%s' at %d Hz: %s", devinfo->name,
        *nominal_sr, err_str);
  return err;
}
 ...
 */
int get_audio_device(void) { return actx.device; }
...
static void scan_audio_device(int i, bool dopulse) {
  ...
  debug("Device %2d: %2d %s%s\n", i, devinfo->maxInputChannels, devinfo->name,
        i == Pa_GetDefaultInputDevice() ? " (default)" : "");
  bool ispulse = !strcmp(devinfo->name, "pulse") ||
                 !strcmp(devinfo->name, "default") ||
                 !strcmp(devinfo->name, "sysdefault");
  if (ispulse ^ dopulse)
    return;
...
static int scan_audio_devices(void) {
  ...
  // Two pass, non-pulse followed by pulse, as scanning a pulseaudio device will
  // make the real device it's using appear busy if it's scanned shortly
  // afterward.
  int i;
  for (i = 0; i < n; i++)
 ...
 *
 * If the selected device is not suitable, perhaps because the audio hardware
 * has changed since the device number was saved, it will fallback to the
 * default device.
 *
 ...
 *
 * The distinction between the nominal and real sample rate is somewhat
 * ill-defined. Nothing uses real sample rate yet.
 *
 * @param device The device to use, or -1 for default.
 * @param[in,out] normal_sample_rate The rate in Hz to use, or 0 for default.
 * Returns actual rate selected.
 ...
 *
 */
int start_portaudio(int device, int *nominal_sample_rate,
                    double *real_sample_rate, struct filter_chain *chain,
                    bool light) {
  ...
  PaDeviceIndex input;
  // Use default input if no device selected or selected device is no longer
  // available.
  if (device < 0 || device >= (int)actx.num_devices ||
      !actx.devices[device].good) {
  ...
  err = set_audio_device(input, nominal_sample_rate, real_sample_rate, chain,
                         light);
...
void fill_buffers(struct processing_buffers *ps) {
  ...
  for (i = 0; i < NSTEPS; i++) {
    ...
    if (len < ps[i].sample_count)
      memcpy(ps[i].samples + len, pa_buffers,
             (ps[i].sample_count - len) * sizeof(*pa_buffers));
  }
}
...
static float *_get_audio_data(uint64_t *start_time, unsigned int len) {
  ...
  const int start =
      (write_pointer + pa_buffer_size - (ts - time)) % pa_buffer_size;
 ...
 *
 * Returns the float value 0-1 scale.  Except it can over 1, not entirely sure
 * why that happens. Returns 0 if the peak meter isn't enabled.
 */
 ...
 *
 * This allocates the buffer when enabled for the first time, but doesn't
 * deallocate when disabling, to avoid races.  Setting it the existing value
 * doesn't do anything.
 */
void set_audio_tppm(bool enable) {
  /* It's ok to assume callback won't be using tppm struct if it has not get
   * been allocated, so it's ok to set it here without a race.  */
  ...
  __sync_synchronize();
  /* This assumes we can atomically set a bool, and that it won't appear set in
   * the callback thread until the initialization, above, is also visible.  */
