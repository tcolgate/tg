...
static int write_hex_double(FILE *f, double d) {
  ...
  if (mantissa && fprintf(f, ".") < 0)
    return 1;
  for (; mantissa; mantissa <<= 4)
    if (fprintf(f, "%c", hex_digit(mantissa >> 60)) < 0)
      return 1;

  int exp = (int)exponent - 1023;
  if (fprintf(f, "p%c", exp < 0 ? '-' : '+') < 0)
    return 1;
...
static int parse_hex_double(FILE *f, double *d) {
  char s[64], *t = s;
  if (1 != fscanf(f, "%63[0-9a-fA-F.xXpP+-]", s))
    return 1;
  double place = 1;
  if (*t == '+')
    t++;
...
static int serialize_int(FILE *f, int x) { return serialize_int64_t(f, x); }

static int scan_int(FILE *f, int *x) {
  int64_t y;
  if (scan_int64_t(f, &y) || y < INT_MIN || y > INT_MAX)
    return 1;
  *x = y;
  return 0;
}
...
static int scan_double(FILE *f, double *x) {
  int n = 0;
#ifdef _WIN32
  if (0 != fscanf(f, " I%n", &n) || !n)
    return 1;
  if (parse_hex_double(f, x))
    return 1;
  n = 0;
  if (0 != fscanf(f, ";%n", &n) || !n)
    return 1;
...
static int serialize_float(FILE *f, float x) { return serialize_double(f, x); }
...
static int serialize_string(FILE *f, char *s) {
  if (0 > fprintf(f, "S%" PRIu64 ";", (uint64_t)strlen(s)))
    return 1;
...
static int scan_string(FILE *f, char **s, uint64_t max_l, uint64_t *len) {
  uint64_t l;
  int n = 0;
  if (1 != fscanf(f, " S%" SCNu64 ";%n", &l, &n) || !n)
    return 1;
  if (max_l && l >= max_l)
    return 1;
  if (!*s)
    *s = malloc(l + 1);
  if (l + 1 != fread(*s, 1, l + 1, f))
    return 1;
  if ((*s)[l] != ';')
    return 1;
  (*s)[l] = 0;
...
static int serialize_uint64_t_array(FILE *f, uint64_t *a, uint64_t len) {
  ...
  if (0 > fprintf(f, "A%" PRIu64 ";\n", len))
    return 1;
  for (i = 0; i < len; i++)
    if (serialize_uint64_t(f, a[i]))
      return 1;
  return 0;
}

static int scan_uint64_t_array(FILE *f, uint64_t **a, uint64_t max_l,
                               uint64_t *len) {
  uint64_t l, i;
  int n = 0;
  if (1 != fscanf(f, " A%" SCNu64 ";%n", &l, &n) || !n)
    return 1;
  if (max_l && l > max_l)
    return 1;
  if (!*a)
...
static int serialize_bool_array(FILE *f, const unsigned char *a, uint64_t len) {
  ...
  if (0 > fprintf(f, "A%" PRIu64 ";\n", len))
    return 1;
  for (i = 0; i < len; i++)
    if (serialize_int(f, a[i]))
      return 1;
  return 0;
}

static int scan_bool_array(FILE *f, unsigned char **a, uint64_t max_l,
                           uint64_t *len) {
  uint64_t l, i;
  int n = 0;
  if (1 != fscanf(f, " A%" SCNu64 ";%n", &l, &n) || !n)
...
static int make_label(FILE *f, char *l) { return 0 > fprintf(f, "L%s;\n", l); }
...
static int serialize_struct_begin(FILE *f) { return 0 > fprintf(f, "T;\n"); }

static int serialize_struct_end(FILE *f) { return make_label(f, "__end__"); }
...
#define SERIALIZE(T, A)                                                        \
  {                                                                            \
    if (make_label(f, #A))                                                     \
      return 1;                                                                \
    if (serialize_##T(f, s->A))                                                \
      return 1;                                                                \
  }

static int serialize_snapshot(FILE *f, struct snapshot *s, char *name) {
  if (serialize_union_begin(f, "realtime-snapshot"))
    return 1;
  if (serialize_struct_begin(f))
    return 1;
  if (name) {
    if (make_label(f, "name"))
      return 1;
    if (serialize_string(f, name))
      return 1;
  }
  if (make_label(f, "pb->waveform"))
    return 1;
  if (serialize_float_array(f, s->pb->waveform, s->pb->sample_count))
    return 1;
  if (make_label(f, "events"))
    return 1;
  if (serialize_uint64_t_array(f, s->events, s->events_count))
    return 1;
  if (make_label(f, "events_tictoc"))
    return 1;
  if (serialize_bool_array(f, s->events_tictoc, s->events_count))
    return 1;
  ...
  if (make_label(f, "amps"))
    return 1;
  if (serialize_float_array(f, s->amps, s->amps_count))
    return 1;
  if (make_label(f, "amps_time"))
    return 1;
  if (serialize_uint64_t_array(f, s->amps_time, s->amps_count))
    return 1;
...
#define SCAN_ARRAY(T, A, count)                                                \
  if (!strcmp(#A, l)) {                                                        \
    ...
    uint64_t x;                                                                \
    if ((*s)->A || scan_##T##_array(f, &(*s)->A, INT_MAX, &x))                 \
      goto error;                                                              \
    count = x;                                                                 \
    continue;                                                                  \
  }
  ...
  *name = NULL;
  ...
  for (;;) {
    ...
    if (!strcmp("pb->waveform", l)) {
      ...
      uint64_t x;
      if ((*s)->pb->waveform ||
          scan_float_array(f, &((*s)->pb->waveform), INT_MAX, &x))
        goto error;
      (*s)->pb->sample_count = x;
      continue;
    }
    if (!strcmp("events", l)) {
      ...
      uint64_t x;
      if ((*s)->events || scan_uint64_t_array(f, &((*s)->events), INT_MAX, &x))
        goto error;
      (*s)->events_count = x;
      continue;
    }
    if (!strcmp("events_tictoc", l)) {
      ...
      uint64_t x;
      if ((*s)->events_tictoc ||
          scan_bool_array(f, &((*s)->events_tictoc), INT_MAX, &x))
        goto error;
  ...
  if ((*s)->pb->period <= 0 || (*s)->pb->sample_count < ceil((*s)->pb->period))
    goto error;
  ...
  if ((*s)->bph && ((*s)->bph < MIN_BPH || (*s)->bph > MAX_BPH))
    goto error;
  debug("serializer: checking la\n");
  if ((*s)->la < MIN_LA || (*s)->la > MAX_LA)
    goto error;
  debug("serializer: checking cal\n");
  if ((*s)->cal < MIN_CAL || (*s)->cal > MAX_CAL)
    goto error;
  debug("serializer: checking events\n");
  if ((*s)->events_count && (*s)->events_wp >= (*s)->events_count)
    goto error;
  if ((*s)->signal > NSTEPS)
    (*s)->signal = NSTEPS;
  ...
  if ((*s)->d->beat_scale == 0)
    (*s)->d->beat_scale = 1.0 / PAPERSTRIP_ZOOM;
  if ((*s)->d->beat_scale < 0 || (*s)->d->beat_scale > 1)
    goto error;
  (*s)->pb->events = NULL;
#ifdef DEBUG
  (*s)->pb->debug = NULL;
  ...
  if ((*s)->events_count) {
    ...
    const uint64_t first =
        (*s)->events[((*s)->events_wp + 1) % (*s)->events_count];
...
static int scan_snapshot_list(FILE *f, struct snapshot ***s, char ***names,
                              uint64_t *cnt) {
  ...
  *names = NULL;
  *cnt = 0;
  if (1 != fscanf(f, " A%" SCNu64 ";%n", &i, &n) || !n)
    goto error;
  *s = malloc(i * sizeof(struct snapshot *));
  *names = malloc(i * sizeof(char *));
  uint64_t j;
  for (j = 0; j < i; j++) {
    if (scan_snapshot(f, *s + *cnt, *names + *cnt))
      goto error;
    *cnt += !!(*s)[*cnt];
  }
...
int write_file(FILE *f, struct snapshot **s, char **names, uint64_t cnt) {
  uint64_t i;
  if (make_label(f, "tg-timer-version"))
    return 1;
  if (serialize_string(f, VERSION))
    return 1;
  if (make_label(f, "data"))
    return 1;
  if (serialize_struct_begin(f))
    return 1;
  if (make_label(f, "snapshot-list"))
    return 1;
  if (0 > fprintf(f, "A%" PRIu64 ";\n", cnt))
    return 1;
  for (i = 0; i < cnt; i++)
    if (serialize_snapshot(f, s[i], names[i]))
      return 1;
  if (serialize_struct_end(f))
    return 1;
  return 0;
}
...
int read_file(FILE *f, struct snapshot ***s, char ***names, uint64_t *cnt) {
  ...
  char *l = lbl;
  if (scan_label(f, l) || strcmp("tg-timer-version", l))
    return 1;
  if (scan_string(f, &l, LABEL_SIZE, NULL))
    return 1;
  debug("serializer: read version %s\n", l);
  if (scan_label(f, l) || strcmp("data", l))
    return 1;
  ...
  *cnt = 0;
  for (;;) {
    ...
    if (!strcmp("snapshot-list", l)) {
      if (*s || scan_snapshot_list(f, s, names, cnt))
        goto error;
      else
        continue;
    }
