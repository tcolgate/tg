...
#define PRESET_BPH                                                             \
  {12000, 14400, 17280, 18000, 19800, 21600,                                   \
   25200, 28800, 36000, 43200, 72000, 0};
...
struct processing_buffers {
  ...
  float *samples, *samples_sc, *waveform, *waveform_sc, *tic_wf, *slice_wf,
      *tic_c;
   ...
   * This might by the same as sample_count, but can be less if the buffer was
   * cloned and only one period of data was saved.  */
  int interval_count;
  ...
  uint64_t timestamp; //< Absolute timestamp of end of buffer, timestamp -
                      //sample_count is time at start of buffer.
  uint64_t last_tic, last_toc, events_from;
  /** Dynamically allocated array of events, using absolute timestamps.
   * Terminated by 0 value for position. */
  struct event *events;
#ifdef DEBUG
  int debug_size;
  float *debug;
...
struct processing_data {
  ...
  uint64_t last_tic;
  int last_step; //!< Guess of step (buffers index) to try first, based on last
                 //!< iteration
  int is_light;
};
...
#define AUDIO_RATE_LABELS                                                      \
  { "22.05 kHz", "44.1 kHz", "48 kHz", "96 kHz", "192 kHz" }
...
struct display {
  ...
  double beat_scale;
  /* Time of point used to anchor the paperstrip.  Each paperstrip point's
   * position is relative to the previous point.  This point is the one with an
   * absolute position that
   * is kept the same, so that all the dots do not shift side to side as the
   * scroll.  */
...
#else
static inline bool python_init(const struct main_window *w) {
  UNUSED(w);
  return true;
}
