...
static gboolean output_cal(GtkSpinButton *spin, gpointer data) {
  ...
  text = g_strdup_printf("%c%d.%d", value < 0 ? '-' : '+', abs(value) / 10,
                         abs(value) % 10);
...
static guint computer_terminated(struct main_window *w) {
  if (w->zombie) {
    ...
    struct computer *c =
        start_computer(w->nominal_sr, w->bph, w->la, w->cal, w->is_light);
    if (!c) {
...
static void handle_tab_changed(GtkNotebook *nbk, GtkWidget *panel, guint x,
  ...
  if (op) {
    gtk_entry_set_text(GTK_ENTRY(w->snapshot_name_entry),
                       tab_name ? tab_name : "");
    bph = op->snst->bph;
    cal = op->snst->cal;
    la = op->snst->la;
...
static GtkWidget *make_tab_label(char *name,
                                 struct output_panel *panel_to_close) {
  GtkWidget *hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
  ...
  if (panel_to_close) {
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(handle_close_tab),
                     panel_to_close);
    gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 0);
    g_object_set_data(G_OBJECT(panel_to_close->panel), "op-pointer",
                      panel_to_close);
    g_object_set_data(G_OBJECT(panel_to_close->panel), "tab-label", label);
    g_object_set_data(G_OBJECT(panel_to_close->panel), "tab-name",
...
static void add_new_tab(struct snapshot *s, char *name, struct main_window *w) {
  ...
  gtk_notebook_insert_page(GTK_NOTEBOOK(w->notebook), op->panel, label,
                           pages - 1);
...
static void handle_snapshot(GtkButton *b, struct main_window *w) {
  UNUSED(b);
  if (w->active_snapshot->calibrate)
    return;
...
static FILE *fopen_check(char *filename, char *mode, struct main_window *w) {
  ...
  name =
      (wchar_t *)g_convert(filename, -1, "UTF-16LE", "UTF-8", NULL, NULL, NULL);
  if (!name)
    goto error;
...
error:
  ...
  if (!f) {
    GtkWidget *dialog;
    dialog = gtk_message_dialog_new(GTK_WINDOW(w->window), 0, GTK_MESSAGE_ERROR,
                                    GTK_BUTTONS_CLOSE, "Error opening file\n");
    gtk_dialog_run(GTK_DIALOG(dialog));
    gtk_widget_destroy(dialog);
  }
...
static FILE *choose_file_for_save(struct main_window *w, char *title,
                                  char *suggestion) {
  FILE *f = NULL;
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      title, GTK_WINDOW(w->window), GTK_FILE_CHOOSER_ACTION_SAVE, "Cancel",
  ...
  if (GTK_RESPONSE_ACCEPT == gtk_dialog_run(GTK_DIALOG(dialog))) {
    ...
    g_object_unref(gf);
    if (!strcmp(".tgj", gtk_file_filter_get_name(
    ...
    if (!stat(filename, &stst)) {
      GtkWidget *dialog = gtk_message_dialog_new(
          ...
          "File %s already exists. Do you want to replace it?", filename);
...
static void save_current(GtkMenuItem *m, struct main_window *w) {
  ...
  if (snapshot->calibrate || !snapshot->pb)
    return;
  ...
  if (f) {
    if (write_file(f, &snapshot, &name, 1)) {
      GtkWidget *dialog =
          gtk_message_dialog_new(GTK_WINDOW(w->window), 0, GTK_MESSAGE_ERROR,
                                 GTK_BUTTONS_CLOSE, "Error writing file");
      gtk_dialog_run(GTK_DIALOG(dialog));
      gtk_widget_destroy(dialog);
    }
    fclose(f);
...
static void save_all(GtkMenuItem *m, struct main_window *w) {
  ...
  for (i = j = 0; i < tabs; i++) {
    ...
    struct output_panel *op = g_object_get_data(G_OBJECT(tab), "op-pointer");
    if (!op)
      continue; // This one is the real-time tab
    s[j] = op->snst;
...
static void load_snapshots(FILE *f, char *name, struct main_window *w) {
  ...
  if (!read_file(f, &s, &names, &cnt)) {
    ...
    GtkWidget *dialog = gtk_message_dialog_new(
        GTK_WINDOW(w->window), 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
...
static void load_from_file(char *filename, struct main_window *w) {
  ...
  if (f) {
    ...
    if (name && strlen(name) > 3 &&
        !strcasecmp(".tgj", name + strlen(name) - 4))
...
static void load(GtkMenuItem *m, struct main_window *w) {
  UNUSED(m);
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      "Open", GTK_WINDOW(w->window), GTK_FILE_CHOOSER_ACTION_OPEN, "Cancel",
...
static GtkWidget *add_checkbox(GtkWidget *menu, const char *name, bool active,
                               GCallback callback, void *param) {
...
static GtkWidget *add_menu_item(GtkWidget *menu, const char *label,
...
static void signal_dialog_response(GtkDialog *dialog, int response_id,
                                   struct main_window *w) {
  UNUSED(w);
...
static void spectrogramt_click(GtkButton *button, struct main_window *w) {
  UNUSED(button);
  spectrogram_time(
      w, gtk_spin_button_get_value(GTK_SPIN_BUTTON(w->spectime_spin)));
}
#endif

static void tppm_meter_active(GtkSwitch *sw, GParamSpec *pspec,
                              struct main_window *w) {
  UNUSED(pspec);
  UNUSED(w);
...
// GtkLevelBar only allows positive values, so bias by 96 dB (16-bit dynamic
// range).  Values under -96 dB aren't very interesting.
static inline float tppm_level(float x) { return x + 96; }

static void init_signal_dialog(struct main_window *w) {
  w->signal_dialog = gtk_dialog_new_with_buttons(
      "Signal", NULL, GTK_DIALOG_DESTROY_WITH_PARENT, "_Close",
      GTK_RESPONSE_CLOSE, NULL);
  gtk_dialog_set_default_response(GTK_DIALOG(w->signal_dialog),
                                  GTK_RESPONSE_OK);
  g_signal_connect(G_OBJECT(w->signal_dialog), "response",
                   G_CALLBACK(signal_dialog_response), w);
  g_signal_connect(G_OBJECT(w->signal_dialog), "delete-event",
                   G_CALLBACK(gtk_widget_hide_on_delete), NULL);

  GtkWidget *content =
      gtk_dialog_get_content_area(GTK_DIALOG(w->signal_dialog));
  ...
  gtk_box_pack_start(GTK_BOX(pbox), gtk_label_new("True Peak Programme Meter"),
                     FALSE, FALSE, 0);

  GtkWidget *tppm_switch = gtk_switch_new();
  gtk_box_pack_start(GTK_BOX(pbox), tppm_switch, FALSE, FALSE, 0);
  g_signal_connect(G_OBJECT(tppm_switch), "notify::active",
                   G_CALLBACK(tppm_meter_active), w);
  g_signal_connect(G_OBJECT(tppm_switch), "map", G_CALLBACK(tppm_meter_map), w);
  g_signal_connect(G_OBJECT(tppm_switch), "unmap", G_CALLBACK(tppm_meter_map),
                   w);
  ...
  w->tppm_level_bar =
      gtk_level_bar_new_for_interval(tppm_level(-70.0), tppm_level(6.0));
  gtk_box_pack_start(GTK_BOX(pbox), w->tppm_level_bar, TRUE, TRUE, 0);
  gtk_orientable_set_orientation(GTK_ORIENTABLE(w->tppm_level_bar),
                                 GTK_ORIENTATION_HORIZONTAL);
  gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(w->tppm_level_bar), "none",
                                 tppm_level(-45.0));
  gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(w->tppm_level_bar),
                                 GTK_LEVEL_BAR_OFFSET_LOW, tppm_level(-30.0));
  gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(w->tppm_level_bar),
                                 GTK_LEVEL_BAR_OFFSET_HIGH, tppm_level(-6.0));
  gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(w->tppm_level_bar),
                                 GTK_LEVEL_BAR_OFFSET_FULL, tppm_level(0.0));
  ...
  gtk_style_context_add_provider(
      gtk_widget_get_style_context(w->tppm_level_bar), GTK_STYLE_PROVIDER(pro),
  ...
  gtk_box_pack_start(GTK_BOX(sbox), gtk_label_new("Spectrograms"), FALSE, FALSE,
                     0);

  GtkWidget *button = gtk_button_new_with_label("Last Tic");
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(spectrogram_click_i),
                   w);
  gtk_box_pack_start(GTK_BOX(sbox), button, FALSE, FALSE, 0);

  button = gtk_button_new_with_label("Last Toc");
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(spectrogram_click_o),
                   w);
  gtk_box_pack_start(GTK_BOX(sbox), button, FALSE, FALSE, 0);
  ...
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(spectrogramt_click),
                   w);
  gtk_box_pack_start(GTK_BOX(sbox), button, FALSE, FALSE, 0);

  w->spectime_spin =
      gtk_spin_button_new_with_range(0.1, 1 << (NSTEPS + FIRST_STEP), 0.1);
  ...
  gtk_box_pack_start(
      GTK_BOX(vbox),
      gtk_label_new("Tg-timer was compiled without Python support so the "
                    "signal plotting functions are unavailble."),
      TRUE, TRUE, 0);
#endif
}

static void fill_current_snapshot_entry(GtkWidget *combo,
...
static void init_main_window(struct main_window *w) {
  ...
  g_signal_connect(w->bph_combo_box, "changed", G_CALLBACK(handle_bph_change),
                   w);
  ...
  g_signal_connect(w->la_spin_button, "value_changed",
                   G_CALLBACK(handle_la_change), w);
  g_signal_connect(w->la_spin_button, "output", G_CALLBACK(handle_la_output),
                   w);
  ...
  g_signal_connect(w->cal_spin_button, "value_changed",
                   G_CALLBACK(handle_cal_change), w);
  ...
  g_signal_connect(w->snapshot_button, "clicked", G_CALLBACK(handle_snapshot),
                   w);

  // Snapshot name field
  GtkWidget *name_label = gtk_label_new("Current snapshot:");
  w->snapshot_name_entry = gtk_combo_box_text_new_with_entry();
  fill_current_snapshot_entry(w->snapshot_name_entry, w->positions,
                              w->positions_size);
  ...
  gtk_box_pack_start(GTK_BOX(w->snapshot_name), w->snapshot_name_entry, FALSE,
                     FALSE, 0);
  gtk_box_pack_start(GTK_BOX(hbox), w->snapshot_name, FALSE, FALSE, 0);
  g_signal_connect(w->snapshot_name_entry, "changed",
                   G_CALLBACK(handle_name_change), w);
  ...
  g_object_set(G_OBJECT(command_menu_button), "direction", GTK_ARROW_DOWN,
               NULL);
  ...
  // ... Save
  w->save_item = add_menu_item(command_menu, "Save current display", false,
                               G_CALLBACK(save_current), w);

  // ... Save all
  w->save_all_item = add_menu_item(command_menu, "Save all snapshots", false,
                                   G_CALLBACK(save_all), w);

  gtk_menu_shell_append(GTK_MENU_SHELL(command_menu),
                        gtk_separator_menu_item_new());

  // ... Light checkbox
  add_checkbox(command_menu, "Light algorithm", w->is_light,
               G_CALLBACK(handle_light), w);

  // ... Calibrate checkbox
  w->cal_button = add_checkbox(command_menu, "Calibrate", false,
                               G_CALLBACK(handle_calibrate), w);

  // Layout checkbox
  add_checkbox(command_menu, "Vertical", w->vertical_layout,
               G_CALLBACK(handle_layout), w);

  // Analysis checkbox
  add_checkbox(command_menu, "Analysis", w->analysis_tab,
               G_CALLBACK(handle_analysis_tab), w);

  gtk_menu_shell_append(GTK_MENU_SHELL(command_menu),
                        gtk_separator_menu_item_new());

  add_menu_item(command_menu, "Signal", true, G_CALLBACK(signal_dialog_show),
                w);

  add_menu_item(command_menu, "Filter Chain", true,
                G_CALLBACK(filter_chain_dialog_show), w);
  w->filter_chain_dialog = filter_dialog_new(w);

  // ... Audio Setup
  w->audio_setup = add_menu_item(command_menu, "Audio setup", true,
                                 G_CALLBACK(audio_setup), w);
  init_audio_dialog(w);

  // ... Close all
  w->close_all_item = add_menu_item(command_menu, "Close all snapshots", false,
                                    G_CALLBACK(close_all), w);
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(w->notebook), w->active_panel->panel,
                           tab_label);
  gtk_notebook_set_tab_reorderable(GTK_NOTEBOOK(w->notebook),
...
static void ppm_update(struct main_window *w) {
  ...
  gtk_level_bar_set_value(GTK_LEVEL_BAR(w->tppm_level_bar),
                          MAX(tppm_level(tppm), 0.0));
}
...
guint refresh(struct main_window *w) {
  ...
  if (s) {
    ...
    // Checked for a pending clear and clear this snapshot if there is one.
    // I.e., the clear was triggered in the small window between when the
    // computer generated the snapshot and this thread received it.
    ...
    if (s->calibrate && s->cal_state == 1 && s->cal_result != w->cal) {
      w->cal = s->cal_result;
      gtk_spin_button_set_value(GTK_SPIN_BUTTON(w->cal_spin_button),
                                s->cal_result);
    }
  }
  unlock_computer(w->computer);
...
static void start_interface(GApplication *app, void *p) {
  ...
  if (start_portaudio(w->audio_device, &w->nominal_sr, &real_sr,
                      w->filter_chain, w->is_light)) {
    g_application_quit(app);
    return;
  }
  w->filter_chain = get_audio_filter_chain();

  if (w->la < MIN_LA || w->la > MAX_LA)
    w->la = DEFAULT_LA;
  if (w->bph < MIN_BPH || w->bph > MAX_BPH)
    w->bph = 0;
  ...
  w->active_panel =
      init_output_panel(w->computer, w->active_snapshot, 0, w->vertical_layout);

  init_main_window(w);

  w->kick_timeout = g_timeout_add_full(G_PRIORITY_LOW, 100,
                                       (GSourceFunc)kick_computer, w, NULL);
  w->save_timeout = g_timeout_add_full(
      G_PRIORITY_LOW, 10000, (GSourceFunc)save_on_change_timer, w, NULL);
#ifdef DEBUG
  if (testing)
...
static void handle_open(GApplication *app, GFile **files, int cnt, char *hint,
                        void *p) {
  UNUSED(hint);
  UNUSED(p);
  struct main_window *w = g_object_get_data(G_OBJECT(app), "main-window");
...
int main(int argc, char **argv) {
  ...
  GtkApplication *app =
      gtk_application_new("li.ciovil.tg", G_APPLICATION_HANDLES_OPEN);
