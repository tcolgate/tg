...
// Get main_window pointer from the tg module object, which is the self pointer
// for any call to a method in the module
const struct main_window *get_w(PyObject *self) {
  ModState *state = PyModule_GetState(self);
  return *state;
}

// Converter function for PyArg_ParseTuple("O&").  Numeric type to a size_t.
// Lets one accept both np.uint64() and int().
...
/* Returns audio as a np.array(dtype=np.float32).  Returns None if the audio
 * isn't in the ring buffer.  The object owns the array data and will free it
 * when the array is GCed.  numpy didn't document how the data is freed exactly,
 * so I hope it's with free().  */
...
// Get most recent length samples of audio.  Returns timestamp of first (oldest)
// sample.
...
// getsamplerate(): float
// Return current sample rate.	This is in the units timestamps use and reflects
// the lite mode decimation (I think) and calibration.
...
// Data is borrowed and object shouldn't be saved across calls without making a
// copy.
...
// Timestamps of the last, as in oldest, tic and toc events still in the
// processing buffer.
static PyObject *get_last_events(PyObject *self, PyObject *noargs) {
  UNUSED(noargs);
  const struct main_window *w = get_w(self);
  const struct snapshot *snst = w->active_snapshot;
  return Py_BuildValue("KK", (unsigned long long)snst->pb->last_tic,
                       (unsigned long long)snst->pb->last_toc);
}
...
// Return what tg considers the beat audio for the one beat at the supplied
// timestamp. The 2nd item is the offset from the start that corresponds to
// timestamp.
static PyObject *get_beat_audio(PyObject *self, PyObject *arg) {
  ...
  const int pad =
      3; // Extra padding, so windowed function can have valid data over SPAN

  if (timestamp == (size_t)-1 && PyErr_Occurred())
    return NULL;

  const unsigned offset = (NEGATIVE_SPAN + pad) * snst->sample_rate / 1000;
  const unsigned ticklen =
      (NEGATIVE_SPAN + POSITIVE_SPAN + pad + pad) * snst->sample_rate / 1000;
  if (timestamp < offset)
...
static PyObject *get_filter(PyObject *self, PyObject *arg) {
  ...
  return Py_BuildValue("(ddd)(ddd)", f->f.a0, f->f.a1, f->f.a2, 1.0, f->f.b1,
                       f->f.b2);
}
...
static PyObject *get_filterchain(PyObject *self, PyObject *noargs) {
  ...
  while ((f = filter_chain_get(w->filter_chain, i++))) {
    if (!f->enabled)
      continue;
    PyList_Append(filters,
                  Py_BuildValue("(ddd)(ddd)", f->f.a0, f->f.a1, f->f.a2, 1.0,
                                f->f.b1, f->f.b2)); // tg swaps a/b from scipy
  }
  return filters;
}

static PyMethodDef methods[] = {
    {"fgcolor", get_fgcolor, METH_NOARGS,
     "Get GtkImage's normal state foreground color"},
    {"getaudio", get_audio, METH_VARARGS,
     "Return nparray of audio data at a timestamp"},
    {"getlastaudio", get_lastaudio, METH_O,
     "Return most recent audio data and timestamp"},
    {"getevents", get_events, METH_NOARGS,
     "Get nparray of current beat timestamps"},
    {"getfilter", get_filter, METH_O, "Get biquad filter coefficients"},
    {"getfilterchain", get_filterchain, METH_NOARGS,
     "Get chain of biquad filter coefficients"},
    {"getfirstevent", get_first_event, METH_VARARGS,
     "Get most revent event as timestamp, phase tuple"},
    {"getlastevents", get_last_events, METH_NOARGS,
     "Return last tic and toc times"},
    {"getsr", get_samplerate, METH_NOARGS, "Get calibrated sample rate"},
    {"getbeataudio", get_beat_audio, METH_O,
     "Get audio for a tick, supply timestamp"},
    {"getpulses", get_pulses, METH_NOARGS,
     "Get length of samples of tic,toc pulses"},
    {}};
 ...
 *
 * Will define a type and a static struct, named "name", that contains fields
 * with names "method1", etc.  that are python objects for those methods as
 * found in the module.
 *
 * Use with LOADMODULE(name), which loads the module code from a global named
 * "name_py" and sets all the pointers.
 */

// For counting the number of arguments in a __VA_ARGS__ list
#define GET_MACRO(_, _1, _2, _3, _4, _5, NAME, ...) NAME
#define COUNT_ARGS(...) GET_MACRO(_, ##__VA_ARGS__, 5, 4, 3, 2, 1, 0)
...
#define FOR_EACH(action, ...)                                                  \
  GET_MACRO(_, ##__VA_ARGS__, FE_5, FE_4, FE_3, FE_2, FE_1, FE_0)              \
  (action, __VA_ARGS__)

// Declare an anonymous union containing an array and a struct.  The struct has
// a field for each variable argument, created by the macro DECL.  The array has
// the specified type and name and a length equal to the number of arguments.
...
#define MODULE(name, ...)                                                      \
  ...
  } name = {.code = name##_py, .name = #name, FOR_EACH(IDINIT, __VA_ARGS__)}
...
#define LOADMODULE(module)                                                     \
  loadmodule((struct emptymod *)&module, MODULE_COUNT(module))
...
#define UNLOADMODULE(module)                                                   \
  unloadmodule((struct emptymod *)&module, MODULE_COUNT(module))
...
// Needs to be 8-bit RGBA and the memview needs to describe the image
// dimensions.  The pixbuf will deref the memview when it's freed.
static GdkPixbuf *create_pixbuf_from_memview(PyObject *memview) {
  ...
  if (mem->ndim != 3 || mem->shape[2] != 4)
    return NULL;
  return gdk_pixbuf_new_from_data(mem->buf, GDK_COLORSPACE_RGB, true, 8,
                                  mem->shape[1], mem->shape[0], mem->strides[0],
                                  del_pyobject, memview);
}

// Set a GtkImage to an image stored in a Python memview.  This steals the
// reference to the memview and gives it to the GtkImage.  Prints error on a
// NULL memview.
...
// Get size of image, as python tuple.  Either allocation or a minimum size.
// Minimum size is set via a custom datum on the image, since Gtk doesn't seem
// to have this concept for GtkImages.
static PyObject *image_size(GtkImage *image) {
  // So lazy, packed into a pointer so I don't have to allocate a struct
  unsigned int dims =
      GPOINTER_TO_UINT(g_object_get_data(G_OBJECT(image), "min-size"));
  int minwidth = (dims & 0xffff), minheight = (dims >> 16) & 0xffff;
  int width = MAX(minwidth, gtk_widget_get_allocated_width(GTK_WIDGET(image))),
      height =
          MAX(minheight, gtk_widget_get_allocated_height(GTK_WIDGET(image)));
...
void create_filter_plot(GtkImage *image, const struct filter *filter, int f0,
                        int Fs, double Q) {
  if (!python_initialized)
    return;

  PyObject *title =
      PyObject_CallFunction(filter_graph.maketitle, "iid", f0, Fs, Q);
  call_plot(image, filter_graph.plotfilter, "(ddd)(ddd)iN", filter->a0,
            filter->a1, filter->a2, // tg's a/b are swapped from scipy
            1.0, filter->b1, filter->b2, Fs, title);
}
...
void spectrogram_beat(struct main_window *w, int which) {
  call_plot(GTK_IMAGE(w->signal_graph), spectrogram.plotspectrogram_beat, "(i)",
            which);
}

void spectrogram_time(struct main_window *w, double length) {
  call_plot(GTK_IMAGE(w->signal_graph), spectrogram.plotspectrogram_time, "(d)",
            length);
}
