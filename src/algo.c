...
void setup_buffers(struct processing_buffers *b) {
  ...
  b->plan_a = fftwf_plan_dft_r2c_1d(2 * b->sample_count, b->samples, b->fft,
                                    FFTW_ESTIMATE);
  b->plan_b = fftwf_plan_dft_c2r_1d(2 * b->sample_count, b->sc_fft,
                                    b->samples_sc, FFTW_ESTIMATE);
  b->plan_c = fftwf_plan_dft_r2c_1d(2 * b->sample_rate, b->waveform, b->sc_fft,
                                    FFTW_ESTIMATE);
  b->plan_d = fftwf_plan_dft_c2r_1d(2 * b->sample_rate, b->sc_fft,
                                    b->waveform_sc, FFTW_ESTIMATE);
  b->plan_e = fftwf_plan_dft_r2c_1d(b->sample_rate, b->tic_wf, b->tic_fft,
                                    FFTW_ESTIMATE);
  b->plan_f = fftwf_plan_dft_r2c_1d(b->sample_rate, b->slice_wf, b->slice_fft,
                                    FFTW_ESTIMATE);
  b->plan_g = fftwf_plan_dft_c2r_1d(b->sample_rate, b->slice_fft, b->slice_wf,
                                    FFTW_ESTIMATE);
...
static void prepare_data(struct processing_buffers *b,
                         int run_noise_suppressor) {
  int i;

  memset(b->samples + b->sample_count, 0, b->sample_count * sizeof(float));
  if (run_noise_suppressor)
    noise_suppressor(b);
...
static double estimate_period(struct processing_buffers *p) {
  ...
  for (fct = 2; first_estimate / fct > p->sample_rate / 12; fct++) {
    int new_estimate =
        peak_detector(p->samples_sc, first_estimate / fct - p->sample_rate / 50,
                      first_estimate / fct + p->sample_rate / 50);
    if (new_estimate > -1 &&
        p->samples_sc[new_estimate] > 0.9 * p->samples_sc[first_estimate]) {
...
static void compute_phase(struct processing_buffers *p, double period) {
  ...
  /* Compute the average of each element in the period for every cycle in the
   * buffer */
  memset(p->waveform, 0, sizeof(*p->waveform) * count);
  /* The i loop has many more iterations than the j loop (~80 to ~9600), so make
   * the i loop the inner loop.  Also this processes the samples sequentially.
   */
  ...
  /* At this point j cycles of the period been summed in waveform, but the last
   * cycle might not have been complete.  i is the position the last cycle
   * stopped at.  */
...
static int compute_parameters(struct processing_buffers *p) {
  int tic_to_toc =
      peak_detector(p->waveform_sc, floor(p->period / 2) - p->sample_rate / 50,
  ...
  if (fabs(shift - p->period / 2) < p->period / 4) {
    p->last_toc =
        p->timestamp - (uint64_t)round(fmod(apparent_phase, p->period));
    int t = p->tic;
    p->tic = p->toc;
    p->toc = t;
    ...
    p->last_toc =
        p->timestamp -
        (uint64_t)round(fmod(p->sample_count - (p->phase + p->toc), p->period));
...
static void do_locate_events(struct event *events, struct processing_buffers *p,
                             float *waveform, int last, int offset, int count,
                             bool tictoc) {
  int i;
  ...
  for (s = p->sample_count - p->sample_rate / 2; s >= 0;
       s -= p->sample_rate / 2) {
    ...
    if (s < last - offset - (count - 1) * p->period - 0.02 * p->sample_rate)
      break;
  }
...
static void locate_events(struct processing_buffers *p) {
  ...
  do_locate_events(events, p, p->waveform + half,
                   (int)(p->last_tic + p->sample_count - p->timestamp), offset,
                   count, true);
  half = p->toc < p->period / 2 ? 0 : round(p->period / 2);
  offset = p->toc - half - (p->toc_pulse - p->tic_pulse) / 2;
  do_locate_events(events + count, p, p->waveform + half,
                   (int)(p->last_toc + p->sample_count - p->timestamp), offset,
                   count, false);
  qsort(events, 2 * count, sizeof(*events),
...
static void compute_amplitude(struct processing_buffers *p, double la) {
  ...
  while (threshold < .2 * glob_max) {
    ...
    if (135 < tic_amp && tic_amp < 360 && 135 < toc_amp && toc_amp < 360 &&
        fabs(tic_amp - toc_amp) < 60) {
      p->amp = (tic_amp_abs + toc_amp_abs) / 2;
      p->tic_pulse = tic_pulse;
      p->toc_pulse = toc_pulse;
      p->be =
          p->period / 2 - fabs(p->toc - p->tic + p->tic_pulse - p->toc_pulse);
      debug("amp: be = %.1f\n", fabs(p->be) * 1000 / p->sample_rate);
      debug("amp = %f°\n", la * p->amp);
      break;
    } else
...
static int add_sample_cal(struct processing_buffers *p,
                          struct calibration_data *cd) {
  int i;
  double phase = -1;
  if (p->waveform_max_i < p->sample_rate * 4 / 10 ||
      p->waveform_max_i > p->sample_rate * 6 / 10) {
...
static void process(struct processing_buffers *p, int bph, double la,
                    int light) {
...
static int process_cal(struct processing_buffers *p,
                       struct calibration_data *cd) {
  prepare_data(p, 0);
...
bool analyze_processing_data(struct processing_data *pd, int step, int bph,
                             double la, uint64_t events_from) {
  struct processing_buffers *p = &pd->buffers[step];

  p->last_tic = pd->last_tic;
  p->events_from = events_from;
  process(p, bph, la, pd->is_light);
  debug("step %d : %f +- %f\n", step, p->period / p->sample_rate,
        p->sigma / p->sample_rate);

  return p->ready;
}

int analyze_processing_data_cal(struct processing_data *pd,
                                struct calibration_data *cd) {
