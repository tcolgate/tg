...
static void compute_update(struct computer *c) {
  ...
  if (step >= 0) {
    debug("%f +- %f\n", ps[step].period / ps[step].sample_rate,
          ps[step].sigma / ps[step].sample_rate);
    ...
    c->actv->is_old = 0;
    /* Signal's range is 0 to NSTEPS, while step is -1 to NSTEPS-1, i.e. signal
     * = step+1 */
...
static void compute_events_cal(struct computer *c) {
  ...
  int i;
  for (i = d->wp - 1; i >= 0 && d->events[i] > s->events[s->events_wp]; i--)
    ;
  for (i++; i < d->wp; i++) {
    if (d->events[i] / s->nominal_sr <= s->events[s->events_wp] / s->nominal_sr)
      continue;
    if (++s->events_wp == s->events_count)
      s->events_wp = 0;
...
static void compute_events(struct computer *c) {
  ...
  if (p && !s->is_old) {
     ...
     * about a period since the last sample.  Place the amplitude measurement at
...
void compute_results(struct snapshot *s) {
  ...
  if (s->pb) {
    s->guessed_bph =
        s->bph ? s->bph : guess_bph(s->pb->period / s->sample_rate);
    s->rate = (7200 / (s->guessed_bph * s->pb->period / s->sample_rate) - 1) *
              24 * 3600;
  ...
  if (s->events_count) {
    const uint64_t time =
        s->timestamp ? s->timestamp : get_timestamp(s->is_light);
    const uint64_t event = s->events[(s->events_wp + 1) % s->events_count];
    if (event) {
      const double beat_length = s->calibrate
                                     ? s->nominal_sr
                                     : (s->sample_rate * 3600) / s->guessed_bph;
...
static void *computing_thread(void *void_computer) {
  ...
  for (;;) {
    ...
    if (calibrate != c->actv->calibrate)
      memset(c->actv->events, 0,
             c->actv->events_count * sizeof(*c->actv->events));
    ...
    if (c->clear_trace) {
      if (!calibrate) {
        ...
        memset(c->actv->amps_time, 0,
               c->actv->amps_count * sizeof(*c->actv->amps_time));
      }
      c->clear_trace = 0;
    }
...
struct computer *start_computer(int nominal_sr, int bph, double la, int cal,
  ...
  for (i = 0; i < NSTEPS; i++) {
    ...
    p[i].interval_count = p[i].sample_count =
        nominal_sr * (1 << (i + first_step));
    setup_buffers(&p[i]);
  }
...
void lock_computer(struct computer *c) { pthread_mutex_lock(&c->mutex); }
